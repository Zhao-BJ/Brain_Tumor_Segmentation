...
def get_arguments():
    ...
    parser.add_argument('--visdom_env', default='BraTS2018_WholeTumor_Crop/SegNet3D')
...
def main():
    ...
    # Data settings
    train_set = BraTS2018_ImgMask_Dataset(data_path=args.data_path, img_modality='all', mask_type='wt')
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        with torch.no_grad():
            ...
            logging.info('Epoch:%3d |WT Dice:%.4f |Dice Loss:%.4f |Entropy Loss:%.4f' % (epoch, wt_dice, dice_loss, entropy_loss))
            vis.line(Y=[wt_dice], X=[epoch], win='wt_dice', update=None if epoch == 1 else 'append', opts=dict(title='wt_dice', legend=['wt_dice']))
